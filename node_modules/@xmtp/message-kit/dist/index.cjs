...
var mlsClient = require('@xmtp/mls-client');
var fs = require('fs');
...
class HandlerContext {
    ...
    static async create(conversation, message, { client, v2client }, commands, commandHandlers, agentHandlers) {
        ...
        context.members = await populateUsernames(conversation.members, client.accountAddress, senderAddress);
        ...
        if (message.contentType.sameAs(contentTypeText.ContentTypeText)) {
            ...
            const attachment = await contentTypeRemoteAttachment.RemoteAttachmentCodec.load(message.content, client);
            content = {
                ...content,
    ...
    async reply(message) {
        ...
        if (this.refConv)
            await this.refConv.send(reply, contentTypeReply.ContentTypeReply);
        else
            await this.conversation.send(reply, contentTypeReply.ContentTypeReply);
    }
    async send(message) {
        if (this.refConv)
    ...
    async react(emoji) {
        ...
        if (this.refConv)
            await this.refConv.send(reaction, contentTypeReaction.ContentTypeReaction);
        else
            await this.conversation.send(reaction, contentTypeReaction.ContentTypeReaction);
    }
...
async function xmtpClient(clientConfig = {}, privateKey = null) {
    ...
    if (!fs__namespace.existsSync(`.cache`)) {
        fs__namespace.mkdirSync(`.cache`);
    }
    const defaultConfig = {
        env: env,
    ...
    const client = await mlsClient.Client.create(account.address, finalConfig);
    ...
    //v2
    const v2client = await xmtpJs.Client.create(wallet, finalConfig);
    // register identity
