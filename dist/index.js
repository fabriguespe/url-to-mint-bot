...
run(async (context) => {
    ...
    const urlPatterns = [
        {
            ...
            transform: (chain, address, tokenId) => `https://xmtp-coinbase-mint-frame.vercel.app/${chain}/${address}/${tokenId}`,
        },
        {
            pattern: /https?:\/\/wallet\.coinbase\.com\/nft\/mint\/eip155:(\d+):erc721:([^:]+)/,
            transform: (chain, address) => `https://xmtp-coinbase-mint-frame.vercel.app/eip55/${chain}/erc721/${address}`,
        },
    ];
    ...
    if (parsedUrl) {
        ...
        await context.send("Error: Unable to parse the provided URL. Please ensure you're sending a valid Zora or Coinbase URL.");
        return;
    }
    await context.send("Hello! I can help you transform from URLs Zora and Coinbase to Mint Frames. Please send a valid URL.");
});
